...
构建成功后，在 `target` 文件夹下找到 `zeppelin-iginx-VERISON.jar` 文件。
...
在 `interpreter` 文件夹下新建一个文件夹`iginx`，将构建好的 `zeppelin-iginx-{VERSION}.jar` 包放入其中即可。
...
将 `zeppelin-iginx-VERISON.jar` 包放入我们准备好的 `zeppelin-interpreter` 文件夹内即可。
